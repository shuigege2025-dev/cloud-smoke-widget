...
.ycs-ghost { border: 0; background: rgba(255,255,255,.06); color: rgba(255,255,255,.7); width: 22px; height: 22px; border-radius: 8px; cursor: pointer; font-size: 13px; line-height: 1; display: flex; align-items: center; justify-content: center; transition: background .15s, transform .12s; }
...
.ycs-ring { position: absolute; width: 36px; height: 36px; margin-left: -18px; margin-top: -18px; pointer-events: none; opacity: 0; animation: ycs-ring-move 2.4s ease-out 0s forwards; will-change: transform, opacity; }
...
function CloudSmokeWidget(props) {
  ...
  const toggleCigarette = () => {
    ...
    setLit(next)
    if (next) { emitSparks(8); emitSmoke(3, 1); showHint('点燃了，慢慢享受') }
    else { emitSmoke(2, 0.6); showHint('掐灭了') }
  }
  ...
  const blowRing = () => {
    ...
    for (let i = 0; i < 2; i++) {
      parts.push({
        ...
        dy: -(90 + Math.random() * 62),
        s: 2.5 + Math.random() * 1.7,
        dur: 2.0 + Math.random() * 0.8,
        delay: i * 0.18,
        rz: (Math.random() * 36 - 18),
        ro: 0.78 + Math.random() * 0.14,
        fa: Math.random() < 0.5
      })
    }
    setRings((r) => r.concat(parts).slice(-16))
    emitSmoke(3, 0.45, 0.3)
    setInhaling(true)
...
return {
  ...
  inject: ['slots', 'timer'],
  apply(ctx) {
    console.log('cloud-smoke-widget: registering shell.overlay entry (v5)')
